...
export class TodoListController {
  ...
  async replaceById(@param.path.string('id') id: string, @requestBody() todoList: TodoList): Promise<void> {
  ...
  */
  @get('/todo-list-and-sub-entities-sql-join/{id}', {
    responses: {
      '200': {
        description: 'TodoList model instance and sub entities(Totod2) using sql join',
        content: {
          'application/json': {
            schema: getModelSchemaRef(TodoList, {includeRelations: true}),
          },
  ...
  async todoListAndSubEntitiesWithSqlJoin(@param.path.string('id') id: string): Promise<TodoListWithRelations | null> {
    return this.todoListRepository.todoListAndSubEntitiesWithSqlJoin(id);
  }
}
